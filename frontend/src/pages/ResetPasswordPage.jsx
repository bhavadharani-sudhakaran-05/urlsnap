...
export default function ResetPasswordPage() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setLoading(false);
      setShake((prev) => prev + 1);
      const msg = err.response?.data?.message || err.response?.data?.error || err.message || 'Failed to reset password. Link may have expired.';
      setError(msg);
    }
  };
        ...
        {!isMobile && (
          <div style={{ position: 'relative', zIndex: 10, marginTop: 'auto', marginBottom: 'auto' }}>
            ...
            <p style={{ fontSize: '15px', fontWeight: 300, color: 'rgba(255,255,255,0.5)', lineHeight: 1.8, maxWidth: '380px', margin: 0 }}>
              Make it strong and memorable. After resetting, you'll be able to log in with your new credentials.
            </p>
          </div>
        )}
      </motion.div>
        ...
        >
          <div style={{ marginBottom: '32px' }}>
            <h2 style={{ fontFamily: "'Playfair Display', serif", margin: 0, color: 'var(--ink, #1C1612)', fontSize: '34px', lineHeight: 1.2 }}>
              <span style={{ fontWeight: 900, display: 'block' }}>Create new password</span>
            </h2>
